...
연결한 뒤에는 같은 명령으로 등록정보, 스크린샷, 업로드된 빌드, 제출 상태까지 검사합니다. 로컬 통과 결과를
스토어 승인 보장처럼 표시하지는 않습니다.

비상업적 목적으로 앱을 테스트하고 있다면 [10분 Release Doctor 파일럿](docs/user-guide/release-doctor-pilot.ko.md)에
참여할 수 있습니다. 자격증명, 비공개 소스, 절대경로, 원본 JSON은 올리지 말고 가린 finding code와 실행
시간만 공유하세요.
