...
```
"내 앱 출시 준비됐어?"
→ Readiness Score 87/100 · 블로커 2개: 스크린샷 6.9인치 없음, What's New 미작성

"마지막 태그 이후 커밋으로 릴리즈 노트 써줘"
→ 한국어/영어 간결·상세·마케팅 3톤 생성 완료 · Play Store 적용할까요?
...
**방법 1 — Remote MCP** (웹 콘솔 계정 필요, 추천)

```bash
# 1. 토큰 발급: https://mimi-seed.pryzm.gg/workspace/api-tokens
# 2. Claude Code에 등록
...
**방법 2 — Local MCP** (Google OAuth, 직접 실행)
...
| 패키지 | 버전 | 설명 |
|--------|------|------|
| [`mimi-seed`](packages/cli) | CLI | `npx mimi-seed init` — 프로젝트 연결 |
