...
| `.mcp.json` | MCP registration | spawns the **local** server: `npx -y @yoonion/mimi-seed-mcp@latest` |
...
`.mcp.json` deliberately resolves the current npm release (`@yoonion/mimi-seed-mcp@latest`), so every newly
spawned server checks the current dist-tag instead of silently reusing an unqualified cached package. An already
running stdio process still cannot update itself; reload the plugin or start a new client thread after a release.
That active-process distinction is the main reason the `mimi-seed-update` skill verifies the runtime version.
