...
작업 순서 자체가 불분명하면 먼저 [Mimi Seed 사용자 가이드](user-guide/README.ko.md)에서 해당 작업을 찾는다.

아래 에러들은 이미 실행 가능한 힌트를 출력한다. 이 문서는 한 줄 힌트가 말해줄 수 없는 것을 보탠다:
**어느 콘솔에서, 누구의 권한으로, 얼마나 기다려야 하는지.**

여기서 시작하라: `mimi-seed doctor` — 모든 로컬 자격증명과 각각의 복구 명령을 보고한다.
...
**도구 호출이 `Transport closed` 로 실패한다.**
모든 호출에서 즉시 이 오류가 나면 클라이언트가 띄운 로컬 stdio 자식 프로세스가 사라졌거나 연결에서 분리된
상태다. 이 메시지만으로 Google·YouTube·스토어 자격증명이 잘못됐다는 뜻은 **아니다**. 이제
`mimi-seed restart mimi-seed` 가 Codex 플러그인 전용 설치를 포함해 클라이언트별 복구 절차를 알려준다.
Claude Code 는 보통 다음 호출에서 다시 연결할 수 있고 `/mcp` 로 상태를 볼 수 있지만, Codex 는 transport가
닫힌 현재 thread에 다시 붙을 수 없다. 새 thread를 시작하거나 Codex를 다시 연 뒤 `mimi_seed_status` 를
호출하라. 그 새 세션에서 인증 전용 오류가 나오기 전에는 재인증하지 마라.

업로드·게시 등 쓰기 도중 연결이 닫혔다면 결과는 **불명**이다. 중복 처리를 막으려면 재시도하기 전에 대상
서비스에서 실제 반영 여부를 먼저 확인하라.
...
1. `mimi-seed doctor` — 전체 자격증명 리포트와 각각의 복구 명령.
