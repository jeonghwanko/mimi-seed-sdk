...
| Threads | 텍스트, 이미지 1장, 영상 1개, 캐러셀 2~20장 | 텍스트 500자, Instagram과 별도 토큰 |
| TikTok Business | 검증된 URL의 공개 영상 | owned Business Account, 로컬 파일 직접 업로드 아님 |

모든 이미지·영상 게시에는 Meta 서버가 읽을 수 있는 **public HTTPS URL**이 필요하다. 로컬 경로, 사내망 URL,
...
```text
https://cdn.example.com/releases/2.4.0/demo.mp4 영상을 Threads에 올릴 게시물로 준비해줘.
계정, 본문, 영상 URL, 대체 텍스트를 확인하고 최종 미리보기를 보여줘. 내가 확인한 뒤에만 게시해.
```
...
플랫폼 앱/페이지에서 최신 게시물을 직접 확인한다. 이미지·영상·캐러셀은 Meta 처리 때문에 몇 초 이상 걸릴 수 있다.
...
- 이미지·영상 URL을 로그아웃 상태에서 열 수 있는지 확인
