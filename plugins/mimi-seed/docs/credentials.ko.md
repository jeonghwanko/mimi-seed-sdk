...
**여러 계정:** `mimi-seed auth instagram --profile <id>`로 이름 있는 계정을 저장한다. 프로젝트의
`.mimi-seed.json`에서 `socialProfiles.instagram`에 해당 ID를 지정한다. 같은 프로필 파일을 Threads와
공유할 수 있지만 플랫폼별 토큰 항목은 분리된다.
