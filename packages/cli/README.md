...
| 명령어 | 설명 |
|--------|------|
| `mimi-seed init` | 프로젝트를 Mimi Seed에 연결 (PAT 발급 + 앱 자동 등록 + `.claude/mimi-seed.md` 주입) |
...
> `init`은 `.claude/mimi-seed.md`를 함께 생성해 Claude Code 세션마다 에이전트 컨텍스트(출시 워크플로우 + 슬래시 커맨드)를 자동 활성화합니다.
