...
환경변수 MIMI_SEED_LANG 가 있으면 그게 우선합니다.`,
  ...
  setup: {
    ...
    tokenExpiring: (days: number) => `토큰 ${days}일 후 만료 — 지금 갱신 권장`,
  ...
  doctor: {
    ...
    credentialMismatch: (field: string, expected: string, actual?: string) =>
      `${field} 불일치 — 요구값 ${expected}, 현재값 ${actual ?? "없음"}`,
...
MIMI_SEED_LANG takes precedence when set.`,
  ...
  setup: {
    ...
    tokenExpiring: (days: number) => `token expires in ${days} day(s) — refresh now`,
  ...
  doctor: {
    ...
    credentialMismatch: (field: string, expected: string, actual?: string) =>
      `${field} mismatch — expected ${expected}, current ${actual ?? "missing"}`,
    credsHint:
