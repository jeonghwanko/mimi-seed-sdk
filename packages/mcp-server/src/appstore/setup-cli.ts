...
const ko = {
  ...
  reconnect: '  API Key를 다시 설정할래? (y/N, v=Vendor Number만): ',
...
const en: typeof ko = {
  ...
  reconnect: '  Replace the API Key? (y/N, v=Vendor Number only): ',
...
async function main() {
  ...
  if (existing) {
    ...
    const intent = await collectExistingSetupIntent(ask, {
      reconnect: M.reconnect,
      vendorNumber: M.askVendorNumber,
    });
    if (!intent.replacePrimaryKey) {
      vendorNumber = intent.vendorNumber;
