...
const receive=(params:Record<string,string>,path='/threads/callback')=>new Promise<number>((resolve,reject)=>{
...
beforeEach(()=>{
 vi.mocked(connectThreads).mockResolvedValue({ok:true,text:'connected'});
 vi.mocked(fetchWithTimeout).mockImplementation(async(url,init)=>{
  if(String(url).endsWith('/start')){start=JSON.parse(String(init?.body));return json({authorizationUrl:'https://broker.example/api/threads-auth/authorize?ticket=example-ticket'});}
...
describe('Threads browser login',()=>{
 ...
 it('권한 거절시 저장하지 않는다',async()=>{
  await expect(connectThreadsInBrowser({...opts,openBrowser:async()=>{await receive({state:start.state,error:'access_denied'});}})).rejects.toMatchObject({code:'denied'});expect(connectThreads).not.toHaveBeenCalled();
 });
 it('시간 초과시 저장하지 않고 서버를 종료한다',async()=>{
  await expect(connectThreadsInBrowser({...opts,timeoutMs:30,openBrowser:async()=>{}})).rejects.toMatchObject({code:'timeout'});expect(connectThreads).not.toHaveBeenCalled();await expect(good()).rejects.toThrow();
 });
 it('미배포 서버 오류에 비밀값을 출력하거나 토큰 입력으로 전환하지 않는다',async()=>{
  vi.mocked(fetchWithTimeout).mockResolvedValue(json({error:'example-secret'},503));await expect(connectThreadsInBrowser(opts)).rejects.toThrow('Threads browser login: unavailable');expect(connectThreads).not.toHaveBeenCalled();
 });
 it('외부 URL과 HTTP 브로커를 거부한다',async()=>{
  vi.mocked(fetchWithTimeout).mockResolvedValue(json({authorizationUrl:'https://evil.example/'}));await expect(connectThreadsInBrowser(opts)).rejects.toMatchObject({code:'configuration'});await expect(connectThreadsInBrowser({...opts,webBase:'http://broker.example'})).rejects.toMatchObject({code:'configuration'});
 });
 it('검증 실패를 성공으로 표시하지 않는다',async()=>{
  vi.mocked(connectThreads).mockResolvedValue({ok:false,text:'example-secret'});await expect(connectThreadsInBrowser({...opts,openBrowser:async()=>{await good();}})).rejects.toThrow('Threads browser login: validation');
 });
 it('자동 브라우저 실행이 실패해도 표시된 URL로 승인할 수 있다',async()=>{
  const onUrl=vi.fn(()=>{void good();});await connectThreadsInBrowser({...opts,onUrl,openBrowser:async()=>{throw new Error('no browser');}});expect(onUrl).toHaveBeenCalled();
 });
});
